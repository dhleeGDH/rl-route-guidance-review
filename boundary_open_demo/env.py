...
State (held fixed across all conditions, non-predictive / local by construction):
  ...
  where c_* are the current-time link costs of the four candidate moves
...
class GridRouteEnv:
    def __init__(self, boundary="closed", reward="time_min", n_side=N_SIDE,
                 ...
                 vanish_potential=False, exit_density=1.0, residual_on_exit=False,
                 state_sentinel="high"):
        assert boundary in ("closed", "open", "costly_return")
        assert reward in ("time_min", "aligned")
        # The state presents an off-grid move as the most expensive action available (a 9.0
        # sentinel) while the reward that move pays is the ordinary step cost of 1.0. Under
        # "matched"
        # the slot carries 1.0 instead, so the state agrees with the reward on every off-grid
        # move. "high" is the published behavior and is the default; nothing else changes,
        # so a cell run under "matched" isolates the sentinel from the objective.
        assert state_sentinel in ("high", "matched")
        self.state_sentinel = state_sentinel
        self.boundary = boundary
        self.reward = reward
        # share of non-destination perimeter links opened on the open condition
        self.exit_density = exit_density
        # The collapse separates the objective from the terminal convention.
        # Leaving costs one step and ends the episode, so the exiting return is about -1 while
        # a completed trip pays for every link. The standard stochastic-shortest-path remedy for
        # an improper policy is to charge the rest of the horizon, and this paper already applies
        # it to the stranded node of Nguyen-Dupuis. With residual_on_exit the same convention
        # applies to a wrong exit: leaving pays the remaining step budget at the minimum in-grid
        # cost, and the exit stays terminal. Nothing else changes, so a cell run with it isolates
        # the convention from the objective.
        self.residual_on_exit = residual_on_exit
    ...
    def _edge_cost(self, r, c, a):
        """Current-time link cost of moving from (r,c) by action a.
    ...
    def _obs(self):
        ...
        for a in range(4):
            ...
            if ec is None:
                ...
                costs.append(UNAVAIL_COST if self.state_sentinel == "high" else 1.0)
            else:
                costs.append(ec)
    ...
    def step(self, a):
        ...
        if not self._in_grid(nr, nc):
            ...
            reward = -1.0
            if self.residual_on_exit:
                # the rest of the horizon, charged at the minimum in-grid link cost of 1.0
                reward -= float(max(0, self.max_steps - self._t - 1))
            if self.reward == "aligned":
