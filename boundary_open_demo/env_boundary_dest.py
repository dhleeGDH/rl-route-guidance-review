...
class BoundaryDestEnv(GridRouteEnv):
    ...
    def _obs(self):
        ...
        for a in range(4):
            ...
            # state_sentinel is set by GridRouteEnv.__init__ and defaults to "high", the
            # published behavior. Under "matched" the slot carries the step cost the reward
            # actually pays for an off-grid move.
            if ec is None:
    ...
    def step(self, a):
        ...
        if not self._in_grid(nr, nc):
            ...
            reward = -1.0
            if getattr(self, "residual_on_exit", False):
                # The exit stays terminal; only the charge changes. Leaving now pays the rest
                # of the horizon at the minimum in-grid cost of 1.0, which is the convention
                # this study already applies to the stranded node of Nguyen-Dupuis and the
                # standard stochastic-shortest-path handling of an improper policy.
