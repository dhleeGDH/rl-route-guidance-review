...
Section V-D scores a policy trained under closure on an opened boundary and reports 17.1% and
40.4%. An off-grid move carries a cost slot of 9.0, above every real cost, and the exit moves
the opened boundary makes available are exactly the moves whose slot holds that value, so the
two numbers may measure what the network does with an input it never met rather than what the
boundary does.
