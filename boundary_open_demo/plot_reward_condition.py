...
boundary changed. Both curves are the ten-seed run of Section V (results_10seed.npz); nothing
is redrawn by hand.
...
(b) OD trip completion, which almost no report states. The boundary-closed agent rises to 85%.
...
d = np.load(HERE / "results_10seed.npz")
steps = d["closed_time_min__steps"]


def band(ax, key, color, label, scale=1.0, marker="o"):
    a = d[key] * scale
    m, s = a.mean(0), a.std(0)
    ax.plot(steps, m, "-", color=color, lw=1.4, marker=marker, ms=2.6,
            markevery=3, label=label)
    ax.fill_between(steps, m - s, m + s, color=color, alpha=0.13, lw=0)
    return m
...
# ---------------- (a) episode return ----------------
mc = band(axA, "closed_time_min__ret", CLOSED, "boundary-closed")
mo = band(axA, "open_time_min__ret", OPEN, "boundary-open", marker="s")
...
co = band(axB, "open_time_min__comp", OPEN, "boundary-open", scale=100.0, marker="s")
axB.set_xlabel("training episodes", fontsize=FS)
axB.set_ylabel("OD trip completion (%)", fontsize=FS)
axB.set_ylim(-6, 108)
axB.grid(True, alpha=0.3, lw=0.5)
axB.legend(loc="center right", frameon=False, fontsize=FS - 1.0, handlelength=1.6)
axB.annotate("never arrives", xy=(steps[-1] * 0.72, 0.0),
             xytext=(steps[-1] * 0.60, 26), fontsize=FS - 1.0, color=OPEN,
             style="italic", ha="center",
             arrowprops=dict(arrowstyle="->", color=OPEN, lw=0.7))
axB.text(steps[-1], cc[-1] + 4, f"{cc[-1]:.0f}%", ha="right", va="bottom",
         fontsize=FS - 0.5, color=CLOSED, fontweight="bold")
axB.text(steps[-1], 3.0, f"{co[-1]:.0f}%", ha="right", va="bottom",
