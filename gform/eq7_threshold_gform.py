...
R_D_BODY = 10.0          # Section IV-B: an arrival bonus of 10 in units of one step's cost
TOL = 1e-9
...
def main():
    ...
    print("%-12s %-8s %10s %4s %6s %6s %12s  %s"
          % ("network", "gamma", "Delta", "K", "k_min", "R_d", "Eq.(7) RHS", "gamma > RHS"))
    for name, path in NETS:
        for g in GAMMAS:
            r = analyse(name, path, g)
            out.append(r)
            print("%-12s %-8s %10.6f %4d %6d %6.1f %12.6f  %s"
                  % (name, g, r["delta"], r["K"], r["k_min"], r["R_d"], r["eq7_rhs"],
                     r["condition_gamma_gt_rhs"]), flush=True)
    json.dump(out, io.open(os.path.join(HERE, "eq7_threshold_gform.json"), "w",
                           encoding="utf-8"), indent=1)
    print("wrote eq7_threshold_gform.json")
