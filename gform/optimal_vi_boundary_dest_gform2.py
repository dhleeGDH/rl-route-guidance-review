# -*- coding: utf-8 -*-
# T-2069 (2026-09-16). The discount-carrying shaping of Eq. (4) on EVERY transition, the exit
# included.
#
# _gform kept the exit transition exactly as the published solver leaves it, which charges no
# shaping there. Section IV-A states the other convention: "On an exit the potential retains the
# value at the link of departure." With Eq. (4) that gives beta*[Phi(l_t) - gamma*Phi(l_t)] =
# beta*(1-gamma)*Phi(l_t) on an exit, which is the increment shaping_m2c.py:59 adds to the
# published environment. Only the exit transition separates this file from _gform; the arrival
# transition (Phi = 0 there) and every boundary-closed figure are untouched, and at gamma = 1 the
# increment vanishes and the published result returns.
#
