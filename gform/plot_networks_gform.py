...
for ax, label in zip(axes, ("(a) 5\u00d75 grid",
    ...
    fig.text(x, 0.012, label, ha="center", va="bottom", fontsize=LABEL_FS)
...
handles = [Line2D([], [], marker="s", ms=3.8, color=BND, ls="none",
           ...
           Line2D([], [], marker="s", ms=2.4, mfc="white", mec=BND, mew=0.7, ls="none",
                  label="node added at the wider border")]
fig.legend(handles=handles, loc="lower center", bbox_to_anchor=(0.5, 0.075), ncol=3,
           frameon=False, fontsize=LABEL_FS, handlelength=1.0, handletextpad=0.4,
