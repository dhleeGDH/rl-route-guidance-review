...
of the corpus a reader could rebuild from the queries the paper states.
  ...
  matching      a corpus title matches a returned record where at least 85% of its distinctive
                tokens appear in that record's title, the rule related_reviews/overlap_refs.py uses
...
Output: exclusion_clause_rerun.csv (one row per corpus study) and exclusion_clause_rerun.json.

    python3 exclusion_clause_rerun.py
    python3 exclusion_clause_rerun.py --offline   # score from a previous fetch, no network
"""
import csv
import io
import json
...
OUT_CSV = os.path.join(HERE, "exclusion_clause_rerun.csv")
OUT_JSON = os.path.join(HERE, "exclusion_clause_rerun.json")
